...
`my setup` configures a *machine*: it ensures the umbrella, persists the
...
Nothing welcomes a *human* who just joined the company, has `my` installed,
...
human-facing intro: an "my by example" walkthrough that explains the model,
...
detail. This is the same altitude rule the rest of the CLI follows — `my`
...
a name: **`my setup`**. We deliberately keep exactly two top-level verbs and
introduce **no** `my configuration` / `my configure` / `my tour`:

- **`my setup`** — the machine configurator. Owns durable `.my-cli/state.json`.
...
- **`my onboard`** — the human tour. Teaches by example, branches on its own
...
1. **Two top-level verbs only.** `my setup` (configure) and `my onboard`
   (teach). No `my configuration`/`configure`/`tour`/`learn`. The "run the
   configurator again later" need is served by re-running `my setup`
   (optionally `my setup --interactive`).

2. **`my setup --interactive`.** An explicit flag adds press-enter prompting
   for the inputs setup currently takes via flags (role; and manifest
   selection — see Decision 6). Plain `my setup` keeps its exact current
...
4. **Reclaim `onboard`.** Drop the deprecated `case "onboard"` dispatch and
   its `warnDeprecated("my onboard", "my setup")` call; remove the
...
5. **Onboard branches on tour-completion.**
   ...
   - *After completion*: a concise "you're already onboarded" review with
     ...
     Re-running the configurator is `my setup` / `my setup --interactive`;
     the tour offers to invoke setup interactively **only on explicit
     confirmation**, never silently.

6. **Manifest selection in interactive flow.** `my setup` today errors when
...
7. **Role selection and clearing.** The role prompt defaults (bare Enter / EOF)
   ...
   `my setup` already treats roles-declared + no-selection as legal and
...
8. **Minimal tour state, umbrella-local only.** Store
   ...
   umbrella `.my-cli/state.json`. **No global/home-level state.** Because the
   ...
   **soft-notifies** ("tour updated, run `my onboard`") — no forced re-tour in
...
10. **Onboard works from zero — by printing, not mutating.** `my setup`
    hard-errors without a registered manifest. `my onboard` runs *before*
    that: with no manifest it explains the model and **prints/copies the exact
    `my manifests add <org> <git-url>` command**, then exits with the tour
...
11. **Onboard flag surface.** `my onboard` supports only `--home`,
...
12. **Two-pane model is the combined model.** Onboard narrates in one console
    ...
    then sends the user to the sandbox pane to try `my roles list`,
    `my services list`, `my customers list`, etc.

13. **Minimal v1 scope.** The tour covers: the model (control vs data plane),
    mounts/services/roles/skills, the common verb set, what `my setup` will
...
removing the `onboard` alias in Step 0 makes `my onboard` unknown until Step
...
- **Step 0 — rename refactor (no behavior change).** Rename internal
  ...
  `my setup` behaves exactly as before; suite stays green.

- **Step 1 — `my setup --interactive`.** Add `app.stdin` (Decision 9) and a
...
- **Step 2 — `my onboard` tour.** Real `--help` surface and the flag set of
...
- **Step 3 — docs.** New site guide page (e.g. `site/guide/onboarding.md`),
  README mention, `skills/my/SKILL.md` note. (Roadmap + plans index are
...
- `my onboard` has a real help surface and is no longer a setup alias.
- `my setup` (no flags) remains scriptable: no prompts, no TTY dependence,
...
- Tour-completion marker round-trips through umbrella `.my-cli/state.json` without
...
- No `my configuration`/`configure`/`tour`/`learn` verb.
- No auto-prompt from plain `my setup` (deferred, Decision 2).
...
- Decision 3 EOF synthesis accepted: defaults are allowed only where the prompt
  ...
  names `--manifest`.
- Tour state is a nested `tour` object inside umbrella `.my-cli/state.json`:
