...
builds and the Windows `our update` fix.

## Context

v0.28.0 added `windows/amd64` + `windows/arm64` release archives and made
`our update` work on Windows (extract `our.exe`; replace the running binary via
...
- **Tests invoke `our` instead of `our.exe`.** Some end-to-end/CLI tests build
  or exec the binary by the bare name `our`, which does not resolve on Windows.
- **CLI print/quoting expectations.** Launch/command preview output (e.g. the
  `our ai --print` shell line) assumes POSIX shell quoting and path style.

## Scope when picked up

- Make the Go test suite pass on Windows: normalize path comparisons via
  `filepath`, escape paths in JSON fixtures, exec `our.exe` where needed.
