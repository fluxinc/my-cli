...
| Plan | Status | Notes |
|---|---|---|
| [2026-06-13-data-surfaces](2026-06-13-data-surfaces.md) | active (Slice 1 shipped v0.22.0) | Slice 1 moved customers from manifest `catalog/customers.json` to mounted `customers/*.md` workspace records. Remaining slices keep business-noun commands stable over thin **data bindings** to existing `mount`/`service` primitives; surface bundles, service-backed domains, and recursive claw-pod materialization remain active. No RBAC/grants in the CLI. Supersedes the vetoed RBAC "Path B" draft. |
