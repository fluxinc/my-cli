...
| Plan | Status | Notes |
|---|---|---|
| [2026-06-13-data-surfaces](2026-06-13-data-surfaces.md) | active (Slice 1 shipped v0.22.0; Slice 2 shipped v0.23.0) | Slice 1 moved customers from manifest `catalog/customers.json` to mounted `customers/*.md` workspace records. Slice 2 adds thin **data bindings** from stable business nouns to existing `mount`/`service` primitives and removes the vestigial service `grant` field. Surface bundles, service-backed domains, and recursive claw-pod materialization remain active. No RBAC/grants in the CLI. Supersedes the vetoed RBAC "Path B" draft. |
